...
class AdminController extends Controller
{
    ...
    public function AdminProfile(){
        ...
        $researchProfile = DB::table('research_profiles')->where('user', "=", Auth::user()->user_id)->get();

        return view('admin.profile', compact('profileData', 'researchProfile'));
    }//end method
    ...
    public function AdminProfileStore(Request $request){
        ...
        try {
            $data->save();
            Log::info("User profile updated successfully. User ID: {$id}");
            return redirect()->back()->with('success', 'Profile updated successfully.');
        } catch (\Exception $e) {
            Log::error("Failed to update user profile. User ID: {$id}. Error: {$e->getMessage()}");
    ...
    public function AdminUpdatePassword(Request $request){
        ...
        try {
            ...
            Log::info("Password updated successfully. User ID: {auth()->user()->id}");
            return back()->with('success', 'Password changed successfully.');
        } catch (\Exception $e) {
            Log::error("Failed to update password. User ID: {auth()->user()->id}. Error: {$e->getMessage()}");
...
====================================================*/
    ...
    public function AdminNoticeStore(Request $request) {
        ...
        $id_not = Notices::find(DB::table('notices')->max('id'));
        if ($request->file('not_file')) {
            $file = $request->file('not_file');
            $extension = $file->getClientOriginalExtension();
            $notice_id = $id_not->id + 1 + 100000;
            $filename = "notice_".date('YmdHi')."_".$notice_id.".".$extension;
    ...
    public function AdminNoticeUpdate(Request $request){ // show all notice
        $id = Auth::user()->user_id;
        $profileData = User::find($id);
    ...
    public function AdminEditNotice($id){ // Single notice edit mode
    ...
    public function AdminNoticeEdited(Request $request, $id){ //store edited notice
        ...
        if($request->file('not_file')){
            $file = $request->file('not_file');
            $id_not = 100000 + $notice->id;
            $filename = "notice_".date('YmdHi')."_".$id_not.".".$file->getClientOriginalExtension();
...
====================================================*/
    ...
    public function AddEvent(){ // show add event page
        $profileData = User::find(Auth::user()->user_id);
        return view('admin.addEvents', compact('profileData'));
    } // end method


    public function AdminEventStore(Request $request){ // store a new event
        $request->validate([
        ...
        if($request->file('file')){
            ...
            $filename = "events_".date('YmdHi')."_".$id_not.".".$file->getClientOriginalExtension();
            $file -> move(public_path('upload/notices', $filename));
    ...
    public function allEvents(){ // show all events
        $profileData = User::find(Auth::user()->user_id);
    ...
    public function editEvents( $id){ // show single event for edit
        $profileData = User::find(Auth::user()->user_id);
    ...
    public function editedEvent(Request $request, $id){ // store edited event
        $event = Events::find($id);
        ...
        $filename = $event->file;
        $request->validate([
            'file' => 'mimes:jpeg,png,jpg,gif,pdf'
        ]);

        if($request->file('file')){
            $id_not = $event->id + 100000;
            $file = $request->file('file');
            $filename = "events_".date('YmdHi')."_".$id_not.".".$file->getClientOriginalExtension();
    ...
    public function deleteEvent($id){ // delete an event
        $event = Events::find($id);
        $event->delete();
...
public function ShowAllExperience(){
    ...
    $OtherExpe = DB::table('other_experiences')->where('user',"=",Auth::user()->user_id)->get();
    return view('admin.experienceAll', compact('profileData','experiencess', 'OtherExpe'));
}
...
public function OtherExperience(){
    $profileData = User::find(Auth::user()->user_id);
    return view('admin.otherExperienceAdd', compact('profileData'));
}

public function StoreOtherExperience(Request $req){
    $experience = new OtherExperience;
    $experience->experience = $req->message;
    $experience->user = Auth::user()->user_id;
    $experience->save();

    return redirect('/admin/experience/all');
}
...
public function SingleOtherExperience($id){
    ...
    return view('admin.otherExperienceUpdate', compact('profileData','experience'));
}
...
public function AddResearchProfile(){
    $profileData = User::find(Auth::user()->user_id);
    return view('admin.researchProfileAdd', compact('profileData'));
}

// use Illuminate\Support\Facades\Log;

public function StoreResearchProfile(Request $req)
{
    $userId = Auth::user()->user_id;
    try {
        $profile = new ResearchProfile;
        $profile->user = $userId;
        $profile->title = $req->message;
        $profile->save();

        Log::info("User {$userId} successfully added a research profile.");

        return redirect()->back()->with('success', 'Research profile added');
    } catch (\Exception $e) {
        Log::error("User {$userId} failed to add a research profile: " . $e->getMessage());

        return redirect()->back()->with('error', 'Failed to add research profile');
    }
}
...
public function EditResearcProfile($id){
    ...
    if($researchProfile->user != Auth::user()->user_id){
        return redirect()->back()->with('error', "Research Profile does not exist");
    }
    return view('admin.researchProfileEdit', compact('researchProfile', 'profileData'));
